...
df = pd.read_csv(r"C:\Users\hamza lagramez\Desktop\ourdatafin.csv")
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    ...
                    html.Div([
                        html.Label(['distribution de la longueur en fanction de les tweets positives et les tweets negatives']),
                        dcc.Dropdown(
                            id='length',
...
#Histogramm2----------------------------------------------- 
@app.callback(
    Output(component_id='histogramm2', component_property='figure'),
    [Input(component_id='none', component_property='children')])
def display_graph( tweet ):
    positive=df[df.target==4]

    fig = ff.create_distplot(hist_data=[positive.length.values.tolist() ],group_labels=['distribution de longueur des tweets ' ])
    return( fig)
